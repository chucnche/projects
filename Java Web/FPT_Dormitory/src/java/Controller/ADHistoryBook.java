 ...
 * @author Chucnc
 */
public class ADHistoryBook extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(room==null) room="";
        HistoryBookDAO deptDB = new HistoryBookDAO();
        ArrayList<HistoryBook> depts = deptDB.searchHisBooks(room, dobFrom, dobTo);
        ...
        if(check==1){
        request.getSession().setAttribute("HistoryBookDAO",depts);    
        request.getSession().setAttribute("pagecountHB", pagecount);
        }

        request.setAttribute("pageindex", pageindex);
        request.setAttribute("pagesize", pagesize);
        request.getRequestDispatcher("ADHistoryBook.jsp").forward(request, response);
        }
        else
