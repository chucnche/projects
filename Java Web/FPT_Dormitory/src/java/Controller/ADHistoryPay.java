 ...
 * @author Chucnc
 */
public class ADHistoryPay extends HttpServlet {
...
//        if(type.equalsIgnoreCase("0") || type==null) type="";
            HistoryPaymentDAO deptDB = new HistoryPaymentDAO();
            ArrayList<HistoryPayment> depts = deptDB.searchHisPays(room, dobFrom, dobTo);
            ...
            if (check == 1) {
                request.getSession().setAttribute("HistoryPaymentDAO", depts);
                request.getSession().setAttribute("pagecountPay", pagecount);
            }
            request.setAttribute("pageindex", pageindex);
            request.setAttribute("pagesize", pagesize);
            request.getRequestDispatcher("ADHistoryPay.jsp").forward(request, response);
        } else {
